...
<head>
    ...
    <title>baza-firm.com.pl webscrapper</title>
</head>
<body>
    <center>
        <h1>Fotowoltaika</h1>
    ...
    <?php
        ...
        if(isset($_POST['submit']) && $region === 'cała_Polska'){
            ...
            foreach ($fotowoltaika as $link) {
                ...
                $nazwa_firmy = $html->find('div[id="nazwaBox"] h1', 0);
                $email_link = $html->find('div[class="displayInlineBlock txtLeft"] img', 0);
                // $telefon = $html->find('p a[href^=tel:]', 0);
                $url = 'https://www.baza-firm.com.pl' . $email_link->attr['src'];
                // $file = fopen('fotowoltaika.jpg', 'w');
                // file_put_contents('fotowoltaika.jpg', file_get_contents($url));
                // fclose($file); 
                // $ch = curl_init($url);
                // $fp = fopen('fotowoltaika.gif', 'wb');
